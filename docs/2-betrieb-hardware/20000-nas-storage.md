...
4. Exportpfad notieren (z. B. `/volume1/k8s-storage` — der genaue Pfad hängt
   ...
   `argocd/apps/tech/nas-storage/deployment.yaml` (`NFS_SERVER`/`NFS_PATH` sowie
...
Die App `nas-storage` (`argocd/apps/tech/nas-storage/`) deployt dafür den
...
eigene ArgoCD-App (`argocd/apps/prod/tinyteller`) und braucht kein
