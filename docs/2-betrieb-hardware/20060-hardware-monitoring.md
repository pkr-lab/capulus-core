...
| vereinsheim-alarmmonitor (Pi, nur Tailscale) | lokaler `vmagent` pusht per remote_write | — | [vmagent-Rolle](../../ansible/roles/vmagent) (`host`/`kind` ab dem nächsten Lauf) |
...
| `vereinsheim-alarmmonitor` fehlt in den Hardware-Dashboards | bekommt `host`/`kind` erst nach dem nächsten Lauf der `vmagent`-Rolle (`make banana-pi-kiosks`); sein eigenes Dashboard "1002011-pis" ist davon unabhängig |
