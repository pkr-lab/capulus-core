...
> **Stand (2026-09-20):** Umgesetzt sind die „weiteren Pipeline-Ergänzungen“
> `ci.yml`, kubeconform und gitleaks ([f0070](../f-cicd-automatisierung/f0070-ci-lint.md)),
> PR-Pflicht für `main` ([f0090](../f-cicd-automatisierung/f0090-branch-schutz-main.md)), die
> Promotion `entw` → PR an `main` nach grüner CI für alles außer den ENTW-Apps
> ([f0080](../f-cicd-automatisierung/f0080-entw-promotion.md)) und die **Kette ENTW → TECH → PROD
> mit Gesundheits-Gate** für die Versionen der ENTW-Apps ([f00b0](../f-cicd-automatisierung/f00b0-promotion-chain.md)):
> 24 h gesund auf ENTW, 2 h auf TECH, ArgoCD-Status über Tailscale, Smoke-Checks per `curl`,
> TECH optional per Auto-Merge, PROD immer manuell. Abweichend von der Skizze unten läuft sie stündlich
> (statt per Push), überträgt nur Versionsfelder und hat keinen Auto-Rollback. Sie ist gebaut und lokal
> getestet, **aber noch nicht aktiv**: es fehlen ArgoCD-Token, Tailscale-Grant und der erste PoC-Lauf
> (Einrichtung in f00b0).
...
| 0.5 | **Pacman-Doppelrolle** — bewusst *nicht* in dieser Migration entscheiden, nur festhalten, dass sie offen bleibt (siehe [App-Zuordnung](#app-zuordnung-erster-entwurf-stand-argocd_platform_appsargocd_workloads_apps)) | nichts — expliziter Nicht-Blocker |
...
| 1.2 | **ENTW-Cluster aufsetzen** (KVM/libvirt-VM auf `worker-1`, eigene
      ...
      **Vorbeugung:** `network_interface` steht jetzt auf `enp4s0`; die Rolle
      ...
      `false` = Hauptcluster unverändert; später entfernt, ENTW nutzt `argocd_entw_layout`). Die VM ist bewusst **nicht** im
...
- [x] **`argocd/apps/entw/*`-Pfad in `renovate.json` ergänzen** — erledigt 2026-09-20: Renovate
      arbeitet zusätzlich auf dem Branch `entw` und aktualisiert dort nur `argocd/apps/entw/`. Die Regel, die für die
      Pilot-Apps in TECH/PROD abschaltet, ist bewusst noch nicht drin (Schritt 8 der Einrichtung in
      [f00b0](../f-cicd-automatisierung/f00b0-promotion-chain.md#einrichtung)), sonst fehlte die Update-Quelle, solange die Kette nicht läuft.
...
- [x] **Smoke-Test-Schritt** im Health-Gate ergänzen — gebaut 2026-09-20
      (`smoke:` in `argocd/promotion.yaml`, `curl --resolve` gegen die ENTW-Ingress-IP;
      [f00b0](../f-cicd-automatisierung/f00b0-promotion-chain.md)). Aktiv, sobald die Kette läuft.
