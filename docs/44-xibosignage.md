...
| MySQL/CMS-Pod: `Permission denied` auf `/var/lib/mysql` bzw. `/var/www/cms/library` | NFS-Squash-Identität auf dem UGREEN NAS hat sich geändert (siehe [docs/16-nas-storage.md](16-nas-storage.md) und die identische Problemlösung bei wikijs/immich) — `securityContext.runAsUser`/`runAsGroup` in `argocd/apps/xibosignage/values.yaml` an die aktuelle Squash-Identität anpassen |
