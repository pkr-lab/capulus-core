...
`ingress.annotations` in `argocd/apps/platform/monitoring/values.yaml` ist
wieder leer. Die `dlrg-einsatz`-Gruppe existiert weiterhin in lldap (könnte
später für etwas anderes verwendet werden), hat aber aktuell keine
`access_control`-Regel mehr.
