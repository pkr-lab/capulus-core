...
- [x] Aktuellste stabile Authentik-Version gegen `Chart.yaml`/`values.yaml`
      → `image.tag` geprüft (2026-09-15). Achtung, Korrektur: ein direkter
      Sprung von `2025.8.3` auf `2026.8.2` schlug live fehl ("Major version
      skips are not allowed") und wurde zurückgesetzt — siehe Rollout-Log
      15.09.2026. `image.tag`/`appVersion` bleiben bewusst auf `2025.8.3`.
...
- [ ] Authentik App-Version schrittweise nachziehen: `2025.8` → `2025.10` →
      `2025.12` → `2026.2` → `2026.5` → `2026.8`, jeder Schritt einzeln
      deployen, Worker-Migration abwarten und verifizieren, bevor der
      nächste Schritt folgt (siehe Rollout-Log 15.09.2026). Vorher: Renovate
      für diesen Chart-Pfad auf Minor-only begrenzen oder Major-PRs manuell
      reviewen, damit sowas nicht wieder automatisch gemerged wird.
- [ ] `authentik-postgres-backup`-CronJob prüfen: alle Dumps der letzten
      9 Tage sind 0 Bytes (siehe Rollout-Log 15.09.2026) — unabhängig vom
      Postgres-18-Upgrade, vermutlich ein vorbestehender Bug.
- [ ] Aufräumen nach Postgres-18-Migration: `pgdata-17-backup`-Verzeichnis
      auf dem `authentik-postgres`-PVC (Rollback-Reserve) sowie den
      manuellen Pre-Upgrade-Dump auf der `authentik-postgres-backup`-PVC
      und in `scratchpad/authentik-pre-pg18-upgrade.dump` löschen, sobald
      der neue Stand als stabil bestätigt ist.
...
| 10.09.2026 | Fix (4) von oben war eine Nebelkerze: `pke`-Login (und alle anderen Apps mit LDAP-Login) schlug weiterhin mit "Invalid password" fehl. Live per `kubectl -n authentik logs deploy/authentik-worker -c worker` verifiziert: das `00-ldap-source.yaml`-Blueprint scheiterte bei **jedem** Apply-Versuch an der Validierung — `"When 'Sync users' is enabled, 'users property mappings' cannot be empty."` (die deployte Authentik-Version, 2025.8.3, verlangt `user_property_mappings`/`group_property_mappings` sobald `sync_users`/`sync_groups` aktiv sind; bei Blueprint-Erstellung fehlten beide Felder komplett). Ein Blueprint wird atomar validiert — die `ldapsource` wurde dadurch **nie** in der DB angelegt (`LDAPSource.objects.filter(slug='lldap')` → leer), also lief nie ein Sync, also existierte `pke` als Authentik-Nutzer schlicht nicht. Authentik antwortet bei unbekanntem Nutzer und falschem Passwort identisch mit "Invalid password" (Anti-Enumeration) — sah daher wie ein Passwortproblem aus, war aber ein "Nutzer existiert nicht"-Problem. Zusätzlich verifiziert (per `ldap3` direkt gegen lldap aus dem Worker-Pod): lldap-Personen-/Gruppen-Einträge haben **kein** AD-typisches `name`-Attribut, nur `cn` — das eingebaute `default-name`-Mapping (`ldap.get("name")`) hätte selbst nach Behebung der Leerfeld-Validierung `None` geliefert und den Gruppen-Sync mit `IntegrityError("Name was not set by propertymappings")` abgebrochen. `user_property_mappings`/`group_property_mappings` in `00-ldap-source.yaml` ergänzt: `openldap-uid` (Username aus `uid`), `openldap-cn` (Name aus `cn`, NICHT `default-name`), `default-mail`, `default-dn-path` für Nutzer; `openldap-cn` für Gruppen. |
| 15.09.2026 | Ein Renovate-Major-PR (`renovate/major-argocdappsplatformauthentik`) hatte auf `main` `postgresql.image.tag` unbeaufsichtigt von `17-bookworm` auf `18-bookworm` gebumpt — ArgoCD synced das automatisch, Postgres crashte dauerhaft (`FATAL: database files are incompatible with server ... initialized by PostgreSQL version 17`), `server`/`worker` hingen im `Init:0/1` fest (Deployment degraded). Parallel dazu war `image.tag` (App-Version) versehentlich direkt von `2025.8.3` auf `2026.8.2` gesetzt worden — Authentik selbst verweigert solche Sprünge (`Major version skips are not allowed`); echte Release-Kette lt. GitHub-Tags: `2025.8 → 2025.10 → 2025.12 → 2026.2 → 2026.5 → 2026.8`. **Postgres-Migration live durchgeführt:** ArgoCD-Sync pausiert (dabei entdeckt: die `authentik`-Application wird von der ApplicationSet `home-server-apps-platform` verwaltet, die ihre Spec inkl. `syncPolicy` laufend neu templated — ein einfacher `kubectl patch` auf die Application allein hält nicht, `argocd-applicationset-controller` musste kurzzeitig mit auf 0 Replicas skaliert werden), Postgres kurz zurück auf `17-bookworm` gegen die bestehenden Daten gestartet, `pg_dump -F c` gezogen (NAS-Backup-PVC + lokale Kopie außerhalb des Clusters), altes `pgdata`-Verzeichnis auf dem PVC per Einmal-Pod zu `pgdata-17-backup` umbenannt (nicht gelöscht — Rollback-Reserve), Postgres auf `18-bookworm` mit frischem `initdb` gestartet, Dump per `pg_restore` eingespielt (178 Tabellen, 9 User, 14 Flows verifiziert). App-Version bewusst auf `2025.8.3` belassen (passend zur wiederhergestellten DB) statt den riskanten 5-Schritte-Sprung live mitzunehmen — das ist ein eigenes, separat zu planendes Vorhaben. **Nebenbefund:** Die automatischen `authentik-postgres-backup`-CronJob-Dumps der letzten 9 Tage sind alle 0 Bytes (unabhängiger, vorbestehender Bug im CronJob — noch nicht untersucht). |
