...
Wrapper Helm chart in `argocd/apps/tech/headlamp/` with the official `headlamp-k8s/headlamp` chart as a dependency. ArgoCD runs `helm dependency build` natively; no extra pipeline steps needed.

### Directory Structure

```
argocd/apps/tech/headlamp/
